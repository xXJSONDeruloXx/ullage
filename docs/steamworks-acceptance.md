...
| 2492670 | METAL GEAR SOLID 4: Guns of the Patriots - Master Collection Version | P | F | P (direct probe only) | I (`gameinstall` available; no round trip) | P | Native Ullage Play reached `Launcher/launcher.exe`, which displayed its own `Application load error 3:0000065432` before `mgs4.exe`. Relay tracing showed the legacy SteamStart shared objects were absent; the separately staged game API probe passed initialization, identity, ownership, and DLC enumeration, but does not certify the shipped game session. A separate GameHub control run reached in-game and was quit normally; that proves the title/runtime combination is viable on this Mac but does not change Ullage's row. |
